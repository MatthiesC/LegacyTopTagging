...
void do_plot(const Year & year) {
  ...
  const string inputDir = (string)getenv("CMSSW_BASE")+"/src/UHH2/LegacyTopTagging/Analysis/ControlPlots/HOTVR_JetPtResponse/";
  const string inputFileName = (string)"HOTVR_JetPtResponse_" + kYears.at(year).name + ".root";
  ...
  mg->Draw("AL3");
  mg->SetTitle("");
  mg->GetHistogram()->GetXaxis()->SetTitle("#it{p}_{T}^{ptcl} [GeV]");
  // mg->GetHistogram()->GetYaxis()->SetTitle("#LT #it{p}_{T}^{reco} / #it{p}_{T}^{ptcl} #GT");
  mg->GetHistogram()->GetYaxis()->SetTitle("#LT #it{R} #GT, #it{R} = #it{p}_{T}^{reco} / #it{p}_{T}^{ptcl}");
  ...
  mg2->Draw("AL3");
  mg2->SetTitle("");
  mg2->GetHistogram()->GetXaxis()->SetTitle("#it{p}_{T}^{ptcl} [GeV]");
  ...
  TText *prelim = new TText(coord->ConvertGraphXToPadX((0.05*p_main->GetWh()/p_main->GetWw())), coord->ConvertGraphYToPadY(0.87), "Private work");
  prelim->SetTextAlign(13); // left top
  prelim->SetTextFont(52);
  prelim->SetTextSize(0.03 / p_main_relative_height);
  prelim->SetNDC();
  // prelim->SetTextColor(kWhite);
  prelim->Draw();

  TText *prelim2 = new TText(coord->ConvertGraphXToPadX((0.05*p_main->GetWh()/p_main->GetWw())), coord->ConvertGraphYToPadY(0.79), "(CMS data/simulation)");
  prelim2->SetTextAlign(13); // left top
  prelim2->SetTextFont(52);
  prelim2->SetTextSize(0.03 / p_main_relative_height);
  prelim2->SetNDC();
  ...
  TLatex *eta_text = new TLatex(coord->ConvertGraphXToPadX(1-(0.05*p_main->GetWh()/p_main->GetWw())), coord->ConvertGraphYToPadY(0.75), "#left|#eta^{reco}#right| < 2.5");
  ...
  // Save to disk
  const string plotName = (string)"plot_HOTVR_ptResponse_v2p1_"+kYears.at(year).name + ".pdf";
  const string plotBasePath = "HOTVR_JetPtResponse";
...
void plots_HOTVR_ptResponse_v2p1() {
