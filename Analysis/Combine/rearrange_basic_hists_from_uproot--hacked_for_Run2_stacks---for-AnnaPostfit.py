...
def create_rearranged_hists(variable, tagger, year, wp, pt_bin, do_plot=False, do_hists=True, do_rebinning=True):
    ...
    task_name = '-'.join(['AnnaPostfit', tagger.name, wp.name, pt_bin.name, year, variable])
    outDir = os.path.join(baseDir, task_name)
    os.system('mkdir -p '+outDir)
    outFileName = 'Templates-'+task_name.replace('AnnaPostfit', 'Anna')+'.root'
    if 'both' in channels:
        outFileName = 'Templates-'+task_name.replace('AnnaPostfit', 'Anna')+'-both-postfit.root' #hack
    ...
    if do_plot:
        ...
        for outFolderName in outFolderNames:
            ...
            plotName = 'Plot-customPostfit-'+task_name+'-'+outFolderName+'-'+mscSplitting+('-withLeg' if do_legend else '-noLeg')+('-Preliminary' if nice.text_prelim == 'Preliminary' else '')+'.pdf'
            ...
            if tagger.name == "ak8_t__tau":
                pt_text_string3 = "105 < #it{m}_{SD} < 210 GeV"
            elif tagger.name == "hotvr_t__tau":
                pt_text_string3 = "140 < #it{m}_{jet} < 220 GeV"
            ...
            #pt_text_prepostfit = '#minus SF applied for #tau_{3}/#tau_{2} < 0.68 #minus'
            pt_text_prepostfit = 'SF applied'
            ...
            tlatex_prepostfit = root.TLatex(nice.coord.graph_to_pad_x(0.66), nice.coord.graph_to_pad_y(0.71), pt_text_prepostfit)
            ...
            tline_prepostfit = root.TLine(nice.coord.graph_to_pad_x(0.68), nice.coord.graph_to_pad_y(0.22), nice.coord.graph_to_pad_x(0.68), nice.coord.graph_to_pad_y(0.73))
            #tline_prepostfit = root.TLine(0.68, 0, 0.68, nice.data_hist.GetMaximum()*0.5)
            tline_prepostfit.SetNDC()
            tline_prepostfit.SetLineWidth(2)
            tline_prepostfit.Draw()

            tarrow_prepostfit = root.TArrow(nice.coord.graph_to_pad_x(0.68), nice.coord.graph_to_pad_y(0.68), nice.coord.graph_to_pad_x(0.55), nice.coord.graph_to_pad_y(0.68), 0.02, "|>")
            #tarrow_prepostfit = root.TLine(0.68, 0, 0.68, nice.data_hist.GetMaximum()*0.5)
            tarrow_prepostfit.SetNDC()
            tarrow_prepostfit.SetLineWidth(2)
            tarrow_prepostfit.Draw()
