...
TGraphAsymmErrors create_sig_eff_graph_run2_partnet_w(
  ...
  const int n = e_leo_Tagger == E_leo_Tagger::isTvsQCD ? 4 : 3;

  vector<double> x, y, exl, exh, eyl, eyh;

  for (int i = 0; i < n; i++) {
    double temp_x = e_leo_Tagger == E_leo_Tagger::isTvsQCD ? leo_ptCenter_top[i] : leo_ptCenter_W[i];
    double temp_y = 0.;
    double temp_eyl = 0.;
    double temp_eyh = 0.;
    for (const auto year : kAllYears) {
      ...
      if (e_leo_Tagger == E_leo_Tagger::isTvsQCD)
        temp_y += sf * lumi_factor * values_leo_partnet_top.at(e_leo_WP).at(year)[i];
      else
...
void do_plot_nsubjettiness(const bool bool_prelim) {
  ...
  if(!plot_at_bottom) {
    ...
    TText *prelim = new TText(coord->ConvertGraphXToPadX(0.05), coord->ConvertGraphYToPadY(0.87), "Preliminary"); // Simulation
    prelim->SetTextAlign(13); // left top
    prelim->SetTextFont(52);
    prelim->SetTextSize(0.035);
    prelim->SetNDC();
...
void do_plot_partnet_top(const bool bool_prelim) {
  ...
  double x_offset_general = 8;

  //auto graph_nominal_BkgEff0p001 = TGraph(4, leo_ptCenter_top, values_leo_TvsQCD_BkgEff0p001.at(year).data());
  auto graph_nominal_BkgEff0p001 = create_sig_eff_graph_run2_partnet_w(E_leo_Tagger::isTvsQCD, E_leo_WP::isBkgEff0p001, -x_offset_general);
  ...
  //auto graph_nominal_BkgEff0p005 = TGraph(4, leo_ptCenter_top, values_leo_TvsQCD_BkgEff0p005.at(year).data());
  auto graph_nominal_BkgEff0p005 = create_sig_eff_graph_run2_partnet_w(E_leo_Tagger::isTvsQCD, E_leo_WP::isBkgEff0p005);
  graph_nominal_BkgEff0p005.SetLineWidth(line_width);
  graph_nominal_BkgEff0p005.SetLineColor(kCyan);
  graph_nominal_BkgEff0p005.SetLineStyle(line_style_nominal);
  graph_nominal_BkgEff0p005.SetMarkerColor(kCyan);
  ...
  //auto graph_nominal_BkgEff0p010 = TGraph(4, leo_ptCenter_top, values_leo_TvsQCD_BkgEff0p010.at(year).data());
  auto graph_nominal_BkgEff0p010 = create_sig_eff_graph_run2_partnet_w(E_leo_Tagger::isTvsQCD, E_leo_WP::isBkgEff0p010, x_offset_general);
  graph_nominal_BkgEff0p010.SetLineWidth(line_width);
  graph_nominal_BkgEff0p010.SetLineColor(kBlue);
  graph_nominal_BkgEff0p010.SetLineStyle(line_style_nominal);
  graph_nominal_BkgEff0p010.SetMarkerColor(kBlue);
