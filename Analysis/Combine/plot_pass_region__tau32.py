...
def add_histos(tagger, wp, pt_bin, process='TTbar__MSc_FullyMerged__nominal', max_value=1):

    histo = None
    variable_name = 'output_probejet_' + ('AK8_tau32' if tagger.name.startswith('ak8') else 'HOTVR_tau32')
...
def create_plot_ak8(wp=None, normalized=False):
    ...
    output_file_name = 'plotPassRegion_ak8_' + wp.name + '_tau32'
    if normalized:
