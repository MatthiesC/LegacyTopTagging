...
void do_plot_nsubjettiness(const Year & year) {
  ...
  mg->GetHistogram()->GetXaxis()->SetLimits(200., 1000.);
...
double line_max_y = maximum_saved * 1.1;

TLine* line1 = new TLine(250, 0, 250, 0.5);
...
line2->Draw();
TLine* line3 = new TLine(350, 0, 350, 0.5);
...
line6->Draw();
  ...
  // Save to disk
  string plotName = "plot_sig_eff_in_mc__ak8__"+kYears.at(year).name+".pdf";
...
void sig_eff_in_mc_plotting() {
    ...
    for(const auto year : kAllYears) {
      do_plot_nsubjettiness(year);
      do_plot_partnet_t(year);
  }
